...
fn parse_machine_description<'a, const OFFSET: isize>(input: &mut InputParser) -> Option<(Vector2, Vector2, Vector2)> {
    let machine_descs = input.next_ints::<6>()?;

    let point_a = Vector2::new(machine_descs[0], machine_descs[1]);
    let point_b = Vector2::new(machine_descs[2], machine_descs[3]);
    let destination = Vector2::new(machine_descs[4] + OFFSET, machine_descs[5] + OFFSET);
