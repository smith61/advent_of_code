...
fn input_iterator<'a>(input: &'a str) -> impl Iterator<Item = (Vector2, Vector2)> + 'a {
    input
    .lines()
    .map(|line| {
        ...
        (Vector2::new(s_x, s_y), Vector2::new(b_x, b_y))
    })
}

fn get_ranges(sensors: &[(Vector2, Vector2)], y_index: isize) -> Vec<(isize, isize)> {
    let mut ranges = vec![];
